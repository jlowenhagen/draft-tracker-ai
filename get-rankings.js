 ...
 *
 * SCORING FORMAT:
 * The frontend passes ?format=ppr|half-ppr|standard as a query string
 * parameter (from the league's Settings), and we map that to the matching
 * FFC endpoint. Unknown/missing formats fall back to "ppr".
 */
...
// Maps our internal setting value to FFC's endpoint segment.
const FORMAT_TO_FFC_PATH = {
  ppr: "ppr",
  "half-ppr": "half-ppr",
  standard: "standard",
};
...
exports.handler = async function (event) {
  try {
    const requestedFormat = (event.queryStringParameters && event.queryStringParameters.format) || "ppr";
    const ffcFormat = FORMAT_TO_FFC_PATH[requestedFormat] || "ppr";

    const res = await fetch(
      `https://fantasyfootballcalculator.com/api/v1/adp/${ffcFormat}?teams=12&year=2026`
    );
    ...
    return {
      ...
      body: JSON.stringify({ players, format: ffcFormat, fetchedAt: new Date().toISOString() }),
    };
  } catch (err) {
